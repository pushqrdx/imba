...
TagFlag.prototype.reval = function (cref,vref){
	var prev = this.value().c();
	this._value = OP('=',cref,vref);
	return prev;
};
...
TagAttr.prototype.jsval = function (){
	return this.value() ? this.value().js() : this.quoted();
};
...
Tag.prototype.js = function (jso){
	...
	var scope = self.scope__();
	var calls = [];
	var statics = [];
	
	var parent = o.par; // self.parent
	var content = o.body;
	var bodySetter = self.isSelf() ? "setChildren" : "setContent";
	...
	var ctor = "";
	var pre = "";
	var typ = self.isSelf() ? "self" : ((self.type().isClass() ? self.type().name() : ("'" + self.type()._value + "'")));
	
	if (self.isSelf()) {
		var closure = scope.closure();
		self._reference = scope.context();
		out = scope.context().c();
		...
		var cacher = self.cacher();
		var ref = self.cacheRef();
		var pars = [typ];
		var varRef = null;
		...
		if (o.ivar) {
			...
			if (!o.optim || o.optim == self || parentType != 2 || len$(parent._children) == 1) {
				pre = ("" + (o.path) + " || ");
			};
	...
	if (o.body instanceof Func) {
		...
		if (len$(children) == 1) {
			...
			content = new TagTree(self,o.body);
		};
	};
	
	self.set({treeType: contentType});
	
	var inlineCacheCall = function(val,setter) {
		var ovar = OP('.',self.cacher(),self.cacher().nextSlot());
		var vvar = self.scope__().predeclare('_B');
		var out = ("" + (ovar.c()) + "!=(" + (vvar.c()) + "=" + val + ")&&" + (o.path) + "." + setter + "(" + (ovar.c()) + "=" + (vvar.c()) + ")");
		return out;
		// let val = part.reval(ovar,vvar)
	};
	
	for (var i = 0, items = iter$(self._attributes), len = items.length, part; i < len; i++) {
		part = items[i];
		var out1 = part.js(jso);
		out1 = ("." + AST.mark(part.name())) + out1;
		if (part.isStatic()) {
			statics.push(out1);
		} else if ((part instanceof TagFlag) && self.cacher()) {
			...
			var val = part.reval(ovar,vvar);
			// calls.push(OP('&&',OP('!=',ovar,OP('=',vvar,val),part)).c)
			var out2 = ("" + (ovar.c()) + "!=(" + (vvar.c()) + "=" + val + ")&&" + (o.path) + "." + part.js(jso));
			calls.push(out2);
			// calls.push(OP('&&',OP('!=',ovar,OP('=',vvar,val),part)).c)
			// calls.push(",{o:path}" + part.js(jso))
		} else if (part.jsval) {
			var ovar1 = OP('.',self.cacher(),self.cacher().nextSlot());
			var vvar1 = self.scope__().predeclare('_B');
			var val1 = part.jsval();
			part.setValue(OP('=',ovar1,vvar1));
			var out3 = ("(" + (ovar1.c()) + "!=(" + (vvar1.c()) + "=" + val1 + ")&&" + (o.path) + "." + part.js(jso) + ")");
			calls.push(out3);
		} else {
			calls.push(o.path + out1);
		};
	};
	...
	if (body) {
		...
		if (self.isSelf() && o.optim && contentType == 2 && (content instanceof TagTree)) {
			var k = self.childCacher().c();
		...
		if (bodySetter == 'setChildren' || bodySetter == 'setContent') {
			...
			if (o.body.nonlocals() && target == calls) {
	...
	var shouldEnd = !(self.isNative()) || o.template || calls.length > 0;
	var hasAttrs = Object.keys(self._attrmap).length;
	...
	// @children:length
	if (shouldEnd || self._children.length) {
		
		var commits = [];
		
		for (var i1 = 0, ary = iter$(self._children), len_ = ary.length; i1 < len_; i1++) {
		...
		if (!shouldEnd && o.optim && o.optim != self) {
			self.set({commit: len$(commits) ? commits : ''});
		} else {
			if ((len$(commits) && o.optim) || !(self.isNative()) || hasAttrs || o.template) {
				calls.push(("" + (o.path) + "." + (self.isSelf() ? "synced" : "end") + "(" + args + ")"));
			};
		};
	};
	
	if (self._reference && !(self.isSelf())) {
		out = ("" + (self.reference().c()) + " = " + pre + "(" + (self.reference().c()) + "=" + ctor + ")");
	} else {
		out || (out = ("" + pre + ctor));
	};
	...
	if (calls != statics) {
		if (o.optim && o.optim != self) {
			if (calls.length && o.commit == undefined) { self.set({commit: ("" + calls.join(","))}) };
		} else {
			out = ("(" + out + "),") + calls.join(",");
		};
	};
	...
	if (self._typeNum) {
		self._typeNum.setValue(contentType);
	};
...
Scope.prototype.predeclare = function (name,o){
	if(o === undefined) o = {};
	this._predeclared || (this._predeclared = {});
	return this._predeclared[name] || (this._predeclared[name] = this.declare(name,null,o));
};
